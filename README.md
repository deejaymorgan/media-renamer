...
3. **First launch only.** macOS Gatekeeper blocks the very first open. Clear it
   **once** and it opens normally forever after. Pick whichever suits you (all assume
   the app is in `/Applications` — adjust the path otherwise):
...
> **Why the warning?** The app isn't notarised by Apple. You can also
...
This needs no Apple Developer account. The app is **not notarised**, so recipients
clear Gatekeeper once on first launch (see [Download & run](#download--run)). For a
...
(see the roadmap in [`SPEC.md`](SPEC.md)).
